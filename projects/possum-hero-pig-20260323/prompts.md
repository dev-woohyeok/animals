...
이 장면: 위기 — 오후. 숲 가장자리에서 코요테 2마리가 나타남. 양들이 패닉. Possum이 양 떼 앞에 서서 양들을 전부 언덕 위로 밀어 올림.
...
— 자연스러운 화면 전환 —
(10-15초) 양 떼가 언덕 꼭대기로 올라감. Possum이 언덕 중간에 서서 양들이 전부 올라간 걸 확인. 아래에서 코요테 2마리가 다가오고 있음. 여자가 소리를 지름.

No background music. 양 울음소리, 코요테 울음소리, Possum 달리는 소리, 여자 소리지르는 소리, 바람소리.
```
...
```
// Story: 전체: 농장에서 버려진 미니 돼지 Possum이 양 몰이를 배움 → 코요테 2마리가 양 떼를 노림 → Possum이 양을 언덕 위로 피신시키고 혼자 돌아가 코요테에 맞섬 → 귀 하나를 잃지만 양 전부 구함 → 수술 후 회복 → 양 몰이 복귀.
앞 장면: 코요테 2마리가 숲에서 나타났다. Possum이 양 떼를 전부 언덕 위로 피신시켰다. 코요테가 다가오고 있다.
이 장면: 클라이맥스 — 양 떼를 안전한 곳에 올려보낸 Possum이 돌아서서 언덕 아래 코요테를 향해 전속력으로 달려감. 15kg 돼지가 코요테 2마리를 향해 돌진.
...
코요테 2마리. 회색-갈색 거친 털. 날카로운 노란 눈. 야위고 날렵한 체형. 언덕 아래에서 위를 올려다봄.

30대 초반 백인 여성의 손. 카키색 작업 조끼 소매 보임. (1인칭 POV 촬영자 — 멀리서 촬영)

(0-1초) 참조 이미지 프레임.
— 1초에 빠른 화면 전환 —
(1-5초) 언덕 꼭대기. Possum이 양 떼 앞에 서 있다. 뒤의 양들은 안전. 아래에서 코요테 2마리가 올라오고 있다. Possum이 코요테를 바라봄.
— 자연스러운 화면 전환 —
(5-10초) Possum이 돌아선다. 양 떼를 등지고. 코요테를 향해. 짧은 다리로 언덕 아래를 향해 전속력으로 달려감. 작은 검은 몸이 코요테를 향해 돌진. 여자가 울면서 소리를 지름.
— 자연스러운 화면 전환 —
(10-15초) 멀리서. Possum이 코요테 2마리 앞에 서서 맞서고 있다. 작은 몸이 뒤로 밀리지 않음. 카메라 심하게 흔들림. 여자 울음소리.

No background music. Possum 달리는 소리, 코요테 울음소리, 여자 울면서 소리지르는 소리, 바람소리.
```
