...
(0-1초) 참조 이미지 프레임.
— 1초에 빠른 화면 전환 —
(1-5초) 농장 비포장 도로변. 걸어가다 풀숲에서 약한 울음소리. 카메라가 소리 방향으로 향함. Luke가 먼저 풀숲 쪽으로 코를 들이밈.
— 자연스러운 화면 전환 —
(5-10초) 손으로 풀을 헤침. 풀 사이에 손바닥만 한 아기 너구리가 웅크리고 울고 있다. 검은 눈 마스크가 보임. 카메라 흔들림.
— 자연스러운 화면 전환 —
(10-15초) 남자가 아기 너구리를 두 손으로 조심스럽게 들어올림. 손바닥 위의 작은 너구리가 떨며 울음. Luke가 옆에서 코를 들이밀고 킁킁거림. 남자 한숨.

No background music. 아기 너구리 울음소리, 풀 헤치는 소리, Luke 킁킁거리는 소리, 남자 한숨, 바람소리.
```
