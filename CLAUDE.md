...
7. **Hook First**: Scene 1은 가장 충격적인 장면 (시간순 아님, 서브샷 없이 단일 프레임)
8. **멀티샷 (Scene 2+)**: 최대 3개 서브샷 × 최대 5초씩 (5초 초과 절대 금지)
9. **스타트 프레임 (0-1초) 즉시 컷 전환**: (0-1초) 전환 beat를 별도 라인으로 작성! 스타트 프레임에서 즉시 완전히 다른 구도로 전환. (0-5초)에 합치지 말 것! (머무르면 고정됨!)
12. **서브샷 5초 규칙**: 서브샷 최대 3개, 각 최대 5초 (절대 초과 금지)
10. **Sora2 자유도**: 카메라는 상세하게, 상황은 간결하게 (과도한 묘사 금지)
11. **1인칭 POV 촬영자 = 등장인물**: 손, 옷소매, 목소리가 보이므로 매 프롬프트에 촬영자 캐릭터 설명 필수 (절대 생략 금지)
13. **60초(1분) 이하**: 총 영상 길이 60초 이하. 장면 4-5개 × 10-15초
