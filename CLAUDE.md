...
7. **시간순 전개**: 모든 장면 시간순 배치 (과거 회상/플래시백 절대 금지). Scene 1은 시간순 도입 장면 (서브샷 없이 단일 프레임)
8. **멀티샷 (Scene 2+)**: 최대 3개 서브샷 × 최대 5초씩 (5초 초과 절대 금지)
9. **서브샷 5초 규칙**: 서브샷 최대 3개, 각 최대 5초 (절대 초과 금지)
...
14. **서브샷 간 전환 프롬프트**: 서브샷 사이에 촬영자의 물리적 카메라 동작 전환 필수 (`— 전환: [동작] —`). 영화식 전환(cut, dissolve, fade) 금지
