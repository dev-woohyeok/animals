...
서브샷 = 개별 미니 씬. 각 서브샷 사이에는 자연스러운 화면 전환을 사용:
```
— 자연스러운 화면 전환 —
```
...
### Scene 1 (도입) - ⚠️ 단일 연속 장면 (서브샷 금지!)
- **Scene 1은 하나의 연속된 장면으로 연출 (서브샷 분할 금지!)**
- (1-15초) 하나의 연속 액션으로 촬영
- 서브샷 전환 없이 카메라가 자연스럽게 따라가는 원테이크
