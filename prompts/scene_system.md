...
서브샷 사이에 촬영자의 물리적 카메라 동작을 반드시 포함:
```
— 전환: [촬영자가 폰을 돌리는/내려놓는/줌인하는 등 물리적 동작] —
```
영화식 전환(cut, dissolve, fade) 금지. 실제 핸드폰 촬영 동작으로만.
...
- **Scene 1 = 도입: 서브샷 없이 단일 프레임 (시간순 첫 장면)**
  ...
  "scenes": [
    {
      ...
      "description": "시간순 도입 장면 - 서브샷 없이 단일 프레임으로 구성",
      "emotion": "감정 키워드",
      "camera": {
