...
**모든 프로젝트에서 이미지 생성 프롬프트를 반드시 포함해야 한다.**
Seedance 2.0은 @Image 참조로 캐릭터 일관성을 확보하므로, 참조 이미지가 없으면 영상 생성 불가.

```yaml
image_prompts:
  - id: "@Image1"
    purpose: "동물 캐릭터 참조 — 초기 상태 (Scene 1~중반 공통)"
    prompt: "[영어 이미지 생성 프롬프트]"
    prompt_kr: "[한국어 번역]"

  - id: "@Image2"
    purpose: "동물 캐릭터 참조 — 회복/변화 후 상태 (후반부용)"
    prompt: "[영어 이미지 생성 프롬프트]"
    prompt_kr: "[한국어 번역]"

  - id: "@Image3+" (선택)
    purpose: "장면별 배경/환경 참조"
    prompt: "[영어 이미지 생성 프롬프트]"
    prompt_kr: "[한국어 번역]"
```
...
5. **한국어 번역 필수**: 모든 이미지 프롬프트에 한국어 해석 포함 (사용자 이해용)
6. **배경 포함**: 캐릭터가 놓일 환경/배경도 함께 묘사
7. **카메라 앵글 명시**: close-up, medium shot 등 샷 타입 지정
