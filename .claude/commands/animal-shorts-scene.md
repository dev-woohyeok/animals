...
5. **자막**: 쇼츠용 짧은 캡션 (영문 + 한글)
6. **설명**: 무엇이 일어나는지
7. **감정**: 이 장면의 감정 톤
8. **서브샷(sub_shots)**: 최대 3개 beats
   - beat 번호
   - time (시간 범위)
   - shot_type (샷 타입)
   - description (설명)
9. **카메라**: 주 움직임 + 서브샷 전환 스타일
10. **조명**: 조명 설정 및 분위기
11. **end_frame**: 이 장면의 마지막 프레임 설명 (다음 장면 시작 이미지 참조용)
12. **start_frame_ref**: (Scene 2+) 전 씬 end_frame 참조
